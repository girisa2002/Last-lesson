...

{
    ...
    public float minDistance = 1.0f; // Минимальное расстояние камеры от персонажа

    public LayerMask collisionMask; // Слой объектов, с которыми камера будет сталкиваться
...

    {
        ...
        Vector3 camDesiredPosition = transform.rotation * new Vector3(0f, 0f, -minDistance);

        camDesiredPosition += player.position + new Vector3(0, offsetDistanceY, 0);
        ...
        if (Physics.Raycast(player.position, (camDesiredPosition - player.position).normalized, out hit, minDistance + 0.1f, collisionMask))

        {

            // Если луч столкнулся, помещаем камеру в точку столкновения
...

        {
            ...
            // Если препятствий нет, перемещаем камеру в желаемое положение
